...
function resolve_stream_observable(stream_defs_by_id: UnresolvedStreamDefMap, stream_def: UnresolvedStreamDef): ResolvedStreamDef {
	...
	if (_.isSymbol(generator)) {
		switch (generator) {
			...
			case OPERATORS.merge:
			case OPERATORS.zip:
				return resolve_stream_from_operator(stream_defs_by_id, {...stream_def, generator})
...
function resolve_streams(stream_defs_by_id: UnresolvedStreamDefMap, unresolved_stream_defs: UnresolvedStreamDef[]): UnresolvedStreamDef[] {
...
function auto(stream_definitions: { [k: string]: any }): SubjectsMap {
